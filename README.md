...
The feature set is designed so that no single column dominates model predictions. Two specific risks were identified and mitigated:

**Risk 1 — `hospitalizations_post_start` dominance:** Hospitalization after drug start is correlated with ADR but is not specific — patients are hospitalized for many reasons unrelated to the drug. In data generation, hospitalization counts are generated with substantial noise and many zero values even for ADR patients, keeping this feature's individual weight below ~12%.

**Risk 2 — `drug_symptom_co_mention` dominance:** Clinical notes mention many symptoms unrelated to the monitored drug. The `negated_symptom_count` feature acts as a counterbalance, and the co-mention signal is generated with high noise to reflect real-world ambiguity.

The NLP-extracted features from both parties (5 from the pharma company via Comprehend Medical, 5 from the health insurer via Comprehend Medical + HealthScribe) add independent signal that distributes weight across the feature set. No single feature is expected to exceed ~12% importance, consistent with well-distributed ADR prediction models in the literature.

### Latent ADR Model (Data Generation)

The synthetic data uses two semi-independent latent risk dimensions per patient — a pharma-side risk score (drug exposure profile, known safety signals) and an insurer-side risk score (real-world outcomes, comorbidity burden) — with moderate correlation (~0.3). The `has_adr` label is derived from a weighted combination of both scores (45% pharma + 45% insurer + 10% noise), ensuring that neither party alone can predict ADR well. This design forces the ML model to leverage features from both parties for optimal performance.

Higher pharma-side risk drives:
- Higher known risk scores and longer treatment duration
- More reported symptoms and higher symptom severity in spontaneous reports
- Shorter time to onset and more concomitant drugs reported

Higher insurer-side risk drives:
- More ER visits and lab abnormalities post drug start
- Higher comorbidity index and prior hospitalization history
- More symptom mentions in clinical notes and chief complaint ADR flags
